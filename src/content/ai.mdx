...
```text
你要在一台 Linux 服务器上部署 Cmonitor，一个 Rust 写的服务器探针。
文档：https://cmonitor-document.pages.dev/

## 你要知道的前提

- Cmonitor 由三部分组成：hub（面板 + API，装在一台机器上）、Cagent（装在每台被监控的
...
### 第四步：验证
    curl -sI https://hub.example.com/ | head -1             # 期望 200（状态页）
...
### 第五步：告诉我接下来手动做什么
你不要替我做这几件事，只把清单给我：
1. 打开安装输出的面板地址（journalctl -u monitor-hub | grep 'Sign in' 随时能找回），
   用应急密码登录
...
4. 节点在线后，可点终端图标通过 Cagent 打开具有完整 root 权限的 Web Terminal；不需要填写 SSH 密码或私钥

### 第六步：如果我给了你节点的安装命令
在目标机器上以 root 执行它，然后：
- systemctl status monitor-agent 确认在跑
- journalctl -u monitor-agent -n 30 看有没有报错
- 确认安装脚本已把 Cagent 服务配置为 root；Cagent v1.1.1 起会拒绝非 root 的终端请求
- 告诉我面板里这个节点几秒后应该就亮了
...
- 不要为 Web Terminal 配置 SSH 密码、私钥或开放 22 端口；它通过在线 Cagent 打开本地 PTY。
- 不要把 agent 服务改成非 root，也不要添加会阻止访问宿主文件系统的沙箱；Web Terminal 依赖 Cagent 以 root 运行。
