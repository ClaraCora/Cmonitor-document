...
面板路径不随文档公开，但它一直在你自己的安装输出里：

```bash
journalctl -u monitor-hub | grep 'Sign in'
```
...
## Web Terminal 需要 SSH 密码或私钥吗

不需要。浏览器连接 Cmonitor，hub 再通过节点已经在线的 Cagent 连接转发终端帧；Cagent 在节点本地
创建 PTY。整个过程不使用 SSH 协议，不要求 `sshd` 或 22 端口，详情见
[Web Terminal](/guide/web-terminal)。

终端由目标节点上的 Cagent 以 root 权限创建。节点离线、Cagent 版本低于 `v1.1.1`、Cagent 没有以
root 运行，或者当前登录会话失效时都不会建立终端。旧安装重新执行节点安装命令即可更新二进制和
服务定义。

## 能加负载告警或插件吗

当前不支持。通知仍只做掉线、流量、到期和登录；hub 也不加载第三方插件。
